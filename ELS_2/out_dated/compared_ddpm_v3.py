...
class GaussianDiffusion(Module):
    ...
    def sample_for_Jaco(self, img, i = 16, j = 16, return_all_timesteps=False):
        (h, w), channels = self.image_size, self.channels
        sample_fn = self.ddpm_sample_Jaco if not self.is_ddim_sampling else self.ddim_sample_Jaco
        return sample_fn(img, i, j, return_all_timesteps=return_all_timesteps)

    def ddpm_sample_Jaco(self, img, i, j, return_all_timesteps=False):
        batch, device, total_timesteps, sampling_timesteps, eta, objective = img.shape[0], self.device, self.num_timesteps, self.sampling_timesteps, self.ddim_sampling_eta, self.objective

        times = torch.linspace(0, 999,
                               steps=sampling_timesteps)  # [-1, 0, 1, 2, ..., T-1] when sampling_timesteps == total_timesteps
        times = list(reversed(times.int().tolist()))
        ...
        for time in tqdm(times, desc='sampling loop time step'):
            time_cond = torch.full((batch,), time, device=device, dtype=torch.long)
            ...
            var = self.posterior_variance[time]
            ...
            for channel in range(pred_noise.shape[1]):
                gradient = torch.autograd.grad(outputs= pred_noise[0, channel, i, j],
                                            inputs=img,
                                            retain_graph=True)[0]

                gradients += gradient

            gradients = gradients / pred_noise.shape[1]
            ...
            img = torch.sqrt(1. / (1 - beta)) * (img - beta * torch.sqrt(1. / (1 - alpha)) * pred_noise) + torch.sqrt(var) * noise
        ...
        return imgs, heatmap_list, noise_list
