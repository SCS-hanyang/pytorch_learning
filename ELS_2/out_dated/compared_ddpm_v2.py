...
class Trainer:
    def __init__(
        ...
        *,
        ...
        self.opt = torch.optim.Adam(
                    model.parameters(),
                    lr=1e-4,           # Initial learning rate as specified
                    betas=(0.9, 0.999),  # Default Adam beta parameters
                    eps=1e-8,            # Default epsilon value for numerical stability
                    weight_decay=0       # No weight decay as specified
                )

        # for logging results in a folder periodically

        gamma = math.pow(0.5, 1 / 50)  # This gives us a per-epoch decay factor

        # Create an exponential learning rate scheduler
        scheduler = torch.optim.lr_scheduler.ExponentialLR(
            optimizer,
            gamma=gamma  # Multiplies learning rate by gamma after each epoch
        )
    ...
    def train(self):
        ...
        for epoch in range(num_epochs):
            ...
            print(f'Epoch {epoch + 1}/{num_epochs}, Loss: {epoch_loss:.4f}, LR: {current_lr:.7f}')
