...
from f import default, Upsample, Downsample, exists, cosine_Alpha, extract
...
def forward_sampling(x, t, Alpha=None, noise=None):
    if noise is None:
    ...
    sqrt_alpha = torch.sqrt(extract(t, Alpha.alphas_cumprod))
    one_minus_alpha = torch.sqrt(1 - extract(t, Alpha.alphas_cumprod))

    return x * sqrt_alpha + noise * one_minus_alpha
...
def reverse_sampling(model, x, t, t_index, Alpha = None):
    ...
    alphas = extract(t, Alpha.alphas)

    sqrt_one_minus_a_cumprod = torch.sqrt(1 - Alpha.alphas_cumprod)

    mean = (x - (1 - alphas) * model(x, t) / sqrt_one_minus_a_cumprod) / torch.sqrt(extract(t, Alpha.alphas))
    ...
    if t_index == 1:
        ...
        noise = torch.randn_like(x)
        posterior_variance = extract(Alpha.posterior_variance)
